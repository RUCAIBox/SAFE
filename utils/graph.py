...
LM_MODEL = RobertaForMaskedLMwithLoss.from_pretrained(path)
device = 'cuda:0'
LM_MODEL.cuda(device); LM_MODEL.eval()
print ('loading done')
    ...
    while cur_idx < n_cids:
        ...
        for j, seq in enumerate(input_ids):
            if len(seq) < max_len:
                ...
                seq = seq[0:max_len]
            input_ids[j] = seq
        input_ids = torch.tensor(input_ids).cuda(device) #[B, seqlen]
...
def get_LM_score_placeholder(cids, question):
    cids = cids[:]
    cids.insert(0, -1) #QAcontext node
    scores = [0]*len(cids)
    # sents, scores = [], []
...
def concepts_to_adj_matrices_2hop_all_pair__use_LM__Part1(data):
    ...
    for qid in qa_nodes:
        for aid in qa_nodes:
            if qid != aid and qid in cpnet_simple.nodes and aid in cpnet_simple.nodes:
                extra_nodes |= set(cpnet_simple[qid]) & set(cpnet_simple[aid])
    extra_nodes = extra_nodes - qa_nodes #one hop nodes between any two concept nodes in qa_nodes
    return (sorted(qc_ids), sorted(ac_ids), question, sorted(extra_nodes))

def concepts_to_adj_matrices_2hop_all_pair__use_LM__Part1_only_qa_cp(data):
    qc_ids, ac_ids, question = data
    qa_nodes = set(qc_ids) | set(ac_ids)
    extra_nodes = set()
    # for qid in qa_nodes:
...
def concepts_to_adj_matrices_2hop_all_pair__use_LM__Part1_qa_and_connect_subgraph_cp(data):
    qc_ids, ac_ids, question = data
    qa_nodes = set(qc_ids) | set(ac_ids)
    # assert len(qa_nodes) > 0
    extra_nodes = set()
    for qid in set(qc_ids):
        for aid in set(ac_ids):
            if qid != aid and qid in cpnet_simple.nodes and aid in cpnet_simple.nodes:
                extra_nodes |= set(cpnet_simple[qid]) & set(cpnet_simple[aid])
    extra_nodes = extra_nodes - qa_nodes #one hop nodes between any two concept nodes in qa_nodes
    return (sorted(qc_ids), sorted(ac_ids), question, sorted(extra_nodes))

def concepts_to_adj_matrices_2hop_all_pair_use_LM_score_filter(data):
    # {'adj': adj, 'concepts': concepts, 'qmask': qmask, 'amask': amask, 'cid2score': cid2score}
    concepts,qmask,amask = data['concepts'], data['qmask'], data['amask']
    concepts = concepts[0:max_node_num]
    qmask = qmask[0:max_node_num]
    amask = amask[0:max_node_num]
    emask = ~(qmask|amask)
    qc_ids = concepts[qmask]
    ac_ids = concepts[amask]
    # num_extra_nodes = max_node_num - (len(qc_ids) + len(ac_ids))
    # extra_nodes = concepts[emask][0:num_extra_nodes] # hyperparameter
    extra_nodes = concepts[emask]
    question = None
    # qc_ids, ac_ids, question = data
    # qa_nodes = set(qc_ids) | set(ac_ids)
    # # assert len(qa_nodes) > 0
...
def concepts_to_adj_matrices_2hop_all_pair__use_LM__Part2(data):
    ...
    # cid2score = get_LM_score_placeholder(qc_ids+ac_ids+extra_nodes, question)
    return (qc_ids, ac_ids, question, extra_nodes, cid2score)

def concepts_to_adj_matrices_2hop_all_pair__use_LM__Part2_mp(data):
    qc_ids, ac_ids, question, extra_nodes = data
    # cid2score = get_LM_score(qc_ids+ac_ids+extra_nodes, question)
    all = qc_ids + ac_ids + extra_nodes
    if len(all) == 0:
        print(question)
    cid2score = get_LM_score_placeholder(qc_ids+ac_ids+extra_nodes, question)
    return (qc_ids, ac_ids, question, extra_nodes, cid2score)
...
def generate_adj_data_from_grounded_concepts__use_LM(grounded_path, cpnet_graph_path, cpnet_vocab_path, output_path, num_processes):
    ...
    with open(grounded_path, 'r', encoding='utf-8') as fin_ground, open(statement_path, 'r', encoding='utf-8') as fin_state:
        ...
        for j, line in enumerate(lines_ground):
            ...
            # QAcontext = "{} {}.".format(statement_obj['statements']['statement'], dic['question']['choices'][0]['text'])
    ...
    with Pool(num_processes) as p:
        ...
        # print("Only save concept nodes in qa context")
        # res1 = list(tqdm(p.imap(concepts_to_adj_matrices_2hop_all_pair__use_LM__Part1_only_qa_cp, qa_data), total=len(qa_data)))
        # print("Only save concept nodes in qa and subgraph context")
        # res1 = list(
        #     tqdm(p.imap(concepts_to_adj_matrices_2hop_all_pair__use_LM__Part1_only_qa_and_subgraph_cp, qa_data), total=len(qa_data)))

    res2 = []
    # print("Not compute the lm score")
    print("Compute the lm score")
...
def generate_adj_data_from_grounded_concepts__use_LM_mp(grounded_path, cpnet_graph_path, qagnn_adj_path, cpnet_vocab_path, output_path, num_processes, extra_node, max_node_num_in_subgraph):
    """
    This function will save
        (1) adjacency matrics (each in the form of a (R*N, N) coo sparse matrix)
        (2) concepts ids
    ...
    global concept2id, id2concept, relation2id, id2relation, cpnet_simple, cpnet, max_node_num
    max_node_num = max_node_num_in_subgraph
    if any(x is None for x in [concept2id, id2concept, relation2id, id2relation]):
        load_resources(cpnet_vocab_path)
    if cpnet is None or cpnet_simple is None:
        load_cpnet(cpnet_graph_path)
    ...
    with Pool(num_processes) as p:
        ...
        # print("Only save concept nodes in qa and subgraph context")
        res1 = list(
            tqdm(p.imap(concepts_to_adj_matrices_2hop_all_pair__use_LM__Part1_only_qa_cp, qa_data), total=len(qa_data)))
        if extra_node == 'only.qa':
            print("Only save nodes in question and answer as subgraph")
            res1 = list(
                tqdm(p.imap(concepts_to_adj_matrices_2hop_all_pair__use_LM__Part1_only_qa_cp, qa_data),
                     total=len(qa_data)))
        else:
    ...
    for j, _data in enumerate(res1):
        if j % 100 == 0: print (j)
        res2.append(concepts_to_adj_matrices_2hop_all_pair__use_LM__Part2_mp(_data))
